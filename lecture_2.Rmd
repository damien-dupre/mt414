...
2/ Open a terminal window and type `quarto check`
...
2/ Open Jupyter Lab and drop the `hello.ipynb` in the list of files (left pane)
...
Unfortunately `.ipynb` or `.py` files cannot be used to build a website. 
...
`.qmd` are actually very similar to `.ipynb` tailored for quarto:
...
4/ In the Terminal window, run the following instructions if it hasn't been done already
...
To publish using GitHub Pages, there are 2 requirements:

1/ The output directory should be changed to a folder named `docs` in the `_quarto.yml` file:
...
2/ Upload and commit on your GitHub repository all the files contained in the website folder and not the website folder itself.

Then in Settings > Pages choose:
- `main` branch
- `/docs` folder
- and Save
